...
void get_input(char * dest){  
	int i=0;  
	printf(">> ");  //optional, tells the user that they're supposed to type something.
	while(1){  
		char c = getchar();  
		if(c == '\n'){  
			dest[i] = 0;  
...
int main(void)
{
	...
	while(1){
		printf("Type 'Y' if you want to start the jig, 'N' for stop. \n");
		get_input(in);  
		printf("%s \n",in);  
